        ...
        if (!empty($this->entryType))
        {
            switch ($this->entryType)
            {
                ...
                case 'publications':
                    ...
                    {
                        $list = ORM::factory('Publication')->where('title_' . $this->language, '<>', '')->where('published', '=', 1)->order_by('date_'.$this->language, 'DESC')->limit($this->limit)->offset($this->offset)->find_all();
