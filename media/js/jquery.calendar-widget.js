(function($) { 
	...
	function calendarWidget(el, params) { 
		...
		var table = '';
			...
			table += ('<div class="calendar-nav"><a href="javascript:prevMonth()" id="prev-month"></a>'+monthNames[month]+'<a href="javascript:nextMonth()" id="next-month"></a></div>');
			// uncomment the following lines if you'd like to display calendar month based on 'month' and 'view' paramaters from the URL
			//table += ('<div class="nav-prev">'+ prev_month +'</div>');
			//table += ('<div class="nav-next">'+ next_month +'</div>');
			table += ('<div class="calendar-table"><table class="calendar-month " ' +'id="calendar-month'+i+' " cellspacing="0">');	
            ...
            for (j=1;j<42;j++){
              ...
              if ((j<firstDay)){
                ...
                table += ('<td class="current-month day'+(j-firstDay+1)+'"><a href="'+document.BASE_URL+'/calendar/event/' + ((params.month + 1 < 10) ? '0'+ (params.month + 1) : (params.month + 1)) + '/' + ((j-firstDay+1 < 10) ? '0'+ (j-firstDay+1) : (j-firstDay+1)) + '" class="day' + (((params.month == thismonth) && (today == (j-firstDay+1))) ? ' is-today' : '') + (((params.month_constant == month) && (day == (j-firstDay+1))) ? ' is-select' : '') + '">'+(j-firstDay+1)+ '</a></td>');
              }
              if ((j - 1)%7==6)  table += ('</tr>');
            }
