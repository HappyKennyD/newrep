    ...
    $(document).ready(function () {
        ...
        $("#enter").click(function () {
            $.ajax({
                ...
                type: "POST",
                dataType: "json",
                data: { text: $('#message').val(), id: '<?=$id?>', guest_email: $('#guest_email').val()},
                            ...
                            '<div class="one-comment">' +
                                ...
                                '<div class="c-user-comment">' +
                                    '<p class="comment-information">' +
                                        '<span>' + data['date'] + ' <?=__('Ваш комментарий находится на модерации')?>' + '</span>' +
                                    '</p>' +
...
<div id="comments-block">
    <?php foreach ($commentaries as $comment): ?>
        <div class="one-comment">
            ...
            <div class="c-user-comment">
                <p class="comment-information">
                    <strong><a href="<?php echo URL::site('profile/view/'.$comment->user->id);?>"><?= HTML::chars($comment->user->show_name()) ?></a></strong><span><?= HTML::chars(Date::translate($comment->date, 'd.m.Y')) ?>, <?= HTML::chars(Date::translate($comment->date, 'H:i')) ?> <?php if (!$comment->status):?> <?=__('Ваш комментарий находится на модерации')?><?php endif?></span>
                </p>
	...
	<input id="guest_email" name="guest_email" maxlenght="60" placeholder="<?=__('Введите Ваш e-mail')?>"/>
